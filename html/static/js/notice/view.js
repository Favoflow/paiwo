...
var NoticeView = {
	...
	NewList: '<div class="notice-list" {{html NoticeView.ActionId(message.photo_id)}}>'+
                    '<div class="notice-user-adv"><a href="/${message.user_host}"><img src="${NoticeView.ActionAvat(message.avatar)}"/></a></div>'+
                    '<div class="notice-list-text">'+
                        '<a class="notice-user-nic" href="/${message.user_host}">${message.nick_name}</a>'+
    ...
    ActionAvat: function(data){
    	var action_avat = '';
    	if(data == ''){
    		action_avat = '/static/images/user_head.gif';
    	}else {
    ...
    ActionImg: function(data){
 		if(data){
 			action_img = '<div class="notice-focus-pic"><img src="http://image.paiwo.co/'+data+'@!280x280"/></div>';
 		}else{
 			action_img = '';
 		}
 		return action_img;
