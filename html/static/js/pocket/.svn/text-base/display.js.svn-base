         ...
         $.ajax({
                url: '/rest',
                 ...
                 success: function(data){
                     ...
                     if(data.error_id == 0){
                        window.location.href = '/'+pd.data.author_domain ;
                     }
                 }
...
pd.getPocket = function(id){
    ...
    $.ajax({
             ...
             async: false,
             data:  {
                   data:JSON.stringify({pocket_id: pd.id,method: 'paiwo.content.pocket.get'})
                    },
             dataType: 'json',
             success: function(data){
                 if(data.error_id == 0){
      ...
      $.ajax({
            url: '/rest',
             ...
             data:  {data:JSON.stringify({
                     ...
                     page_no: page_no,
                     page_size: 100,
                     method: 'paiwo.content.pocket.comment.get'})
                    },
             dataType: 'json',
             success: function(data){
                 if(data.error_id == 0){   
...
pd.sendComment = function(text, reply){
    $.ajax({
            ...
            data:  {data: JSON.stringify({
                     ...
                     reply_user_id:reply,
                     comment_text: text,
                     method: 'paiwo.content.pocket.comment.add'})
                    },
             dataType: 'json',
             success: function(data){
                 if(data.error_id == 0){
...
pd.getCollect = function (){
     $.ajax({
            url: '/rest',
             type: 'POST',
             async: false,
            data:  {data: JSON.stringify({method: 'paiwo.user.favorite.name_list.get'})},
             dataType: 'json',
             success: function(data){
                 if(data.error_id == 0){
                     var tm = '';
...
pd.saveCollect = function(id){
    var data ={};
    data.method = 'paiwo.user.favorite.add';
    data.favorite_id = id;
    data.content_id = pd.data.pocket_id;
    data.content_type = 2;
        $.ajax({
