            ...
            sendMessage: function(message){
                ...
                if(this.ws.readyState == 1){
                 ...
                 this.read();
                    ...
                    PWS.reConnec('ws://message.'+domian+'/message');
...
//                    console.log(soucer);
                    for(var i =0;i<soucer.length; i++){
//                        console.log(data.result.user_list[soucer[i].user_id].avatar);
    ...
    hasNew: function(data){
        ...
        //检测是否有信息未读，出现红点
        if(data.system_dynamic.unread_count >0 || data.system_dynamic.unread_count >0 ||data.system_feed.unread_count >0){
            $('#top_message').find('i').show();
                return;
        }
    
        for(var i = 0; i<data.current_session.length; i++){
            if(data.current_session[i].unread_count !=0){
                $('#top_message').find('i').show();
                return;
            }
        }
    ...
    showNotice: function(object){
        ...
        for(var i = 0; i<object.length; i++){
            ...
            if(t.action == 'editor_like'){
                t.msg = t.msg.replace('{0}', top_data.nick_name);
            }
...
var PWS = PWSocket('ws://message.'+domian+'/message');
...
PWS.on('init', function(data){
    ...
    var _this = this;
//    console.log(data);
//    console.log(JSON.stringify(data.msg.history_session));
    
    if(is_login == 0){return;}
    
    paiwo.hasNew(data.msg);
    data.msg.history_session.reverse();
    
    
    var a = [],
        b = [];
    
//    console.log(data.msg.current_session);
    
    if(data.msg.current_session.length != 0){
        for(var i =0; i<data.msg.current_session.length; i++){
            a.push(data.msg.current_session[i].user_id);
        }
    }
    
    
    if(data.msg.history_session.length != 0){
        for(var i =0; i<data.msg.history_session.length; i++){
            b.push(data.msg.history_session[i].user_id);
        }
    }    
...
//    console.log(data.msg.current_session);
    ...
    cloud.crossAjax({
            ...
            if(_data.error_id == 0){
                ...
                for(var i=0;i<data.msg.current_session.length;i++){
                    data.msg.current_session[i].avatar = _data.response.user_list[data.msg.current_session[i].user_id].avatar;
                    data.msg.current_session[i].nick_name = _data.response.user_list[data.msg.current_session[i].user_id].user_name;
                    data.msg.current_session[i].user_host = _data.response.user_list[data.msg.current_session[i].user_id].domain;
                }
...
//                console.log(tm1);
                paiwo.pushSession(_this.curlist, data.msg.current_session);
                $('#cur_list').html(tm1);
                paiwo.checkScroll(); 
            }    
    });
...
//                console.log(_data);
                ...
                for(var i=0;i<data.msg.history_session.length;i++){
                    data.msg.history_session[i].avatar = _data.response.user_list[data.msg.history_session[i].user_id].avatar;
                    data.msg.history_session[i].nick_name = _data.response.user_list[data.msg.history_session[i].user_id].user_name;
                    data.msg.history_session[i].user_host = _data.response.user_list[data.msg.history_session[i].user_id].domain;
                 }
...
//                console.log(tm2);
                ...
                paiwo.pushSession(_this.hislist, data.msg.history_session);
...
//    }
//    
////    console.log(JSON.stringify(data.msg.current_session));
//    var tm1 = $.tmpl(paiwo.contact, data.msg.current_session);
//    var tm2 = $.tmpl(paiwo.history, data.msg.history_session);
//    $('#cur_list').html(tm1);
//    $('#his_list').html(tm2);
...
//            _margin = parseInt($('#fixed-top-box').css('margin-left'));
//        console.log(width);
